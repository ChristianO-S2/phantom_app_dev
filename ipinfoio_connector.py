...
class IpinfoIoConnector(BaseConnector):

    def __init__(self):

        # Call the BaseConnectors init first
        super(IpinfoIoConnector, self).__init__()
    ...
    def _handle_lookup_ip(self, param):
        ...
        # make rest call
        ret_val, response = self._make_rest_call(
            '/{0}/hostname'.format(ip), action_result, params=None, headers=None
        )
    ...
    def _handle_validate_ip(self, param):
        # Implement the handler here
        # use self.save_progress(...) to send progress messages back to the platform
        self.save_progress("In action handler for: {0}".format(self.get_action_identifier()))
    ...
    def handle_action(self, param):
        ...
        if action_id == 'validate_ip':
            ret_val = self._handle_validate_ip(param)
...
def main():
    ...
    if username and password:
        try:
            login_url = IpinfoIoConnector._get_phantom_base_url() + '/login'
